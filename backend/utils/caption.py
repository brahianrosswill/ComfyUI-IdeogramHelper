...
def _ordered(src: dict, order: list[str]) -> dict:
    """Return a new dict with present keys in canonical order."""
    out: dict[str, Any] = {}
    for k in order:
...
def build_caption(data: dict, warnings: list[str] | None = None, errors: list[str] | None = None) -> dict:
    """Normalize a loose dict into a canonical, key-ordered caption dict."""
    ...
    if style:
        ...
        if has_photo and has_art:
            errors.append("style_description has both 'photo' and 'art_style'; keep exactly one")
            has_art = False
        if not has_photo and not has_art:
            errors.append("style_description present but missing one of 'photo' / 'art_style'")
    ...
    if not background:
        errors.append("compositional_deconstruction.background is required")
    ...
    for i, el in enumerate(raw_elements):
        ...
        bbox = el.get("bbox")
        if bbox:
            e["bbox"] = _clean_bbox(bbox, i, errors)

        if etype == "text":
            text = (el.get("text") or "").strip()
            if not text:
                errors.append(f"elements[{i}]: type 'text' has no 'text' to render")
            e["text"] = text
        desc = (el.get("desc") or "").strip()
        if not desc:
            errors.append(f"elements[{i}]: 'desc' is empty")
        e["desc"] = desc
...
def _clean_bbox(bbox: Any, idx: int, errors: list[str]) -> list[int]:
    try:
        y0, x0, y1, x1 = (int(round(float(v))) for v in bbox)
    except (TypeError, ValueError):
        errors.append(f"elements[{idx}]: malformed bbox {bbox!r}; expected [y0,x0,y1,x1] of 4 numbers")
        return []
...
def validate_caption(data: dict) -> tuple[list[str], list[str]]:
    """Return (hard errors, soft warnings) for a parsed caption dict; never raises."""
    errors: list[str] = []
    warnings: list[str] = []
    build_caption(data, warnings=warnings, errors=errors)
